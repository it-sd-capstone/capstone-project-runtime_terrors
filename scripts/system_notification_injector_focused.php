<?php
/**
 * System Notification Injector (Focused Version)
 * 
 * This script:
 * 1. Creates the logSystemEvent function in a helper file
 * 2. Scans core controllers and models for critical system events
 * 3. Injects logSystemEvent calls only at high-priority locations
 */
...
// Only target specific directories and high-priority events
$focus_directories = [
    '/controllers',
    '/models'
];

// Exclude less critical files to focus on main system functionality
$exclude_patterns = [
    'test_', 
    'Test.php',
    'index.php',
...
// Focus on specific high-priority events
$event_patterns = [
    // User account events
    [
        'pattern' => '/function\s+(?:create|add|register)User\s*\([^)]*\)\s*{/',
        'event_type' => 'user_created',
        'subject' => 'New User Registration',
        'message' => 'A new user account was created',
        'insert_after' => '{',
        'condition' => '$success || $user_id'
    ],
    ...
    [
        ...
        'insert_after' => '{',
        'condition' => '$success || $provider_id'
    ],
    
    // Appointment management
    [
        'pattern' => '/function\s+(?:cancel|delete)Appointment\s*\([^)]*\)\s*{/',
        'event_type' => 'appointment_cancelled',
        'subject' => 'Appointment Cancelled',
        'message' => 'An appointment was cancelled in the system',
        'insert_after' => '{',
        'condition' => '$success'
    ],
    ...
    // System settings
    [
        'pattern' => '/function\s+(?:update|save|change)Settings\s*\([^)]*\)\s*{/',
...
// Get PHP files in the focus directories
$files = [];
foreach ($focus_directories as $dir) {
    $dir_path = $root_dir . $dir;
    if (is_dir($dir_path)) {
        $files = array_merge($files, findPhpFiles($dir_path, $exclude_patterns));
    }
}

echo "Found " . count($files) . " PHP files to scan in focus directories.\n";
 ...
 */
function findPhpFiles($dir, $exclude_patterns = []) {
    $files = [];
    ...
    if (is_dir($dir)) {
        ...
        foreach ($items as $item) {
            ...
            if (is_dir($path) && !in_array($item, $skip_dirs)) {
                $files = array_merge($files, findPhpFiles($path, $exclude_patterns));
